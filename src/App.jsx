      ...
      <div className="auth-card">
        ...
        {tab === "login" ? (
          ...
          <form onSubmit={handleRegister}>
            ...
            <div className="form-group"><label className="form-label">Soy</label><select className="form-select" value={form.rol} onChange={e => set("rol", e.target.value)}><option value="alumno">Alumno</option><option value="docente">Docente</option></select></div>
...
function DocenteView({ profile }) {
  ...
  async function loadData() {
    setLoading(true);
    const [c, mod, t, eq, e, al] = await Promise.all([
      supabase.from("cursos").select("*").order("nombre"),
      supabase.from("modulos").select("*").order("orden"),
  ...
  const TABS = [
    { id: "entregas", label: "Entregas", badge: stats.pendientes },
  ...
  return (
    <main className="main">
      ...
      <div className="grid-4" style={{ marginBottom: 32 }}>
        <div className="stat-card"><div className="stat-num" style={{ color: "var(--amber)" }}>{stats.pendientes}</div><div className="stat-label">Pendientes</div></div>
        <div className="stat-card"><div className="stat-num" style={{ color: "var(--green)" }}>{stats.aprobadas}</div><div className="stat-label">Aprobadas</div></div>
        ...
        <div className="stat-card"><div className="stat-num">{equipos.length}</div><div className="stat-label">Equipos activos</div></div>
      </div>
      ...
      <div className="tabs">
        {TABS.map(t => (
          <button key={t.id} className={`tab-btn ${tab === t.id ? "active" : ""}`} onClick={() => setTab(t.id)}>
            {t.label}
            {t.badge > 0 && <span style={{ marginLeft: 5, fontSize: 11, background: "var(--amber-bg)", color: "var(--amber)", padding: "1px 6px", borderRadius: 10 }}>{t.badge}</span>}
          </button>
        ))}
      </div>
              ...
              <div style={{ display: "flex", gap: 0, marginBottom: 20, borderBottom: "1px solid var(--border)" }}>
                {ETABS.map(et => (
                  <button key={et.id} className={`tab-btn ${entregaTab === et.id ? "active" : ""}`} onClick={() => setEntregaTab(et.id)}>
                    {et.label} {et.n > 0 && <span style={{ marginLeft: 4, fontSize: 11, background: "var(--bg3)", color: "var(--text2)", padding: "1px 5px", borderRadius: 10 }}>{et.n}</span>}
                  </button>
                ))}
              </div>
    ...
    <AuthCtx.Provider value={{ session, profile }}>
      ...
      <div className="app">
        {!session ? <AuthPage /> : !profile
          ...
          : <><Navbar profile={profile} />{profile.rol === "alumno" ? <AlumnoView profile={profile} /> : <DocenteView profile={profile} />}</>}
      </div>
    </AuthCtx.Provider>
  );
}
